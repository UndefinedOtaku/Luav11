exports.run = async (client, msg, args) => {
  let kufur=[
    ...
    "ananın amına bacağımı sokup yarım vole attığımının evladı",
    "ananın adet suyuna ekmek bandıımın",
    "ananın damında hafriyat çalışması yapayım",
    "anası sikişmiş derler mesela. ulen dana senin anan da sikişmiş yoksa sen nasıl doğdun?",
    ...
    "senin karının amının içindeki yarrağını sikerim",
    "seni bir sikerim dokuz ay bulanık sıçarsın",
    "seni domaltır sikmem rencide olursun.",
    "seni sikeceğime 31 çekerim daha iyi",
    "evlat olsan sevilmezsin. 31 olsan çekilmezsin",
    ...
    "amında öss sınavı yaparım 2 milyon kişi girer",
    "ananı tavana asayım, amına smaç basayım",
    "optik mouseu ananın amına sokarım geceleri size lazer gösterisi yapar",
    "keşke zamanında baban ananın amına yanmış odun soksaydı duman çıksaydı da sen çıkmasaydın",
 ...
 description: (`:no_entry_sign: Beni mi kandırcan orospu çocuğu ?`)
  ...
  msg.channel.send({embed: {
 color: Math.floor(Math.random() * (0xFFFFFF + 1)),
 description: (`${member} ${kufur[Math.floor(Math.random() * 35)]}.`)
  }})
  }
